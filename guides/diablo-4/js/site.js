/* ============================================================
   THE ARCANUM - Diablo IV guide - shared site JS
   Ambient rising embers (blood + ember), reveal, nav, top.
   ============================================================ */

/* --- ambient: drifting embers (ember + blood) --- */
(function () {
  ...
  var ember = spr("243,173,94"), blood = spr("226,88,74");
  function mk(init) { return { x: rnd(0, W), y: init ? rnd(0, H) : H + 10, r: rnd(1, 2.8), vy: rnd(7, 18), sway: rnd(.3, .9), amp: rnd(8, 24), ph: rnd(0, 6.28), ember: Math.random() < 0.6 }; }
  function init() { resize(); var n = Math.round(Math.min(52, (W * H) / 32000)); P = []; for (var i = 0; i < n; i++) P.push(mk(true)); }
  function frame(t) {
    ...
    for (var i = 0; i < P.length; i++) { var p = P[i]; p.y -= p.vy * dt; p.ph += p.sway * dt; var x = p.x + Math.sin(p.ph) * p.amp;
      if (p.y < -12) { P[i] = mk(false); continue; } var a = .24 * Math.sin(Math.min((H - p.y) / H, 1) * Math.PI) + .05; var sz = p.r * 7;
      ctx.globalAlpha = a < 0 ? 0 : a; ctx.drawImage(p.ember ? ember : blood, x - sz / 2, p.y - sz / 2, sz, sz); }
    ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over"; raf = requestAnimationFrame(frame);
  }
  function start() { if (!raf) { last = 0; raf = requestAnimationFrame(frame); } }
  function stop() { if (raf) { cancelAnimationFrame(raf); raf = null; } }
  if (reduce) { resize(); ctx.globalCompositeOperation = "lighter"; for (var i = 0; i < 26; i++) { var sz = rnd(5, 12); ctx.globalAlpha = rnd(.05, .14); ctx.drawImage(ember, rnd(0, W), rnd(0, H), sz, sz); } ctx.globalAlpha = 1; }
  else { init(); start(); }
