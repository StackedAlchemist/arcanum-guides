/* ============================================================
   THE ARCANUM - Final Fantasy XVI guide - shared site JS
   Ambient ember sparks (fire + aether), reveal-on-scroll,
   mobile nav, back-to-top.
   ============================================================ */

/* --- ambient: rising ember sparks (fire + aether-blue) --- */
(function () {
  ...
  var ember = spr("255,138,92"), aether = spr("169,208,245");
  function mk(init) { return { x: rnd(0, W), y: init ? rnd(0, H) : H + 10, r: rnd(1, 2.6), vy: rnd(7, 18), sway: rnd(.3, .9), amp: rnd(8, 24), ph: rnd(0, 6.28), cool: Math.random() < 0.2 }; }
  ...
  function frame(t) {
    ...
    for (var i = 0; i < P.length; i++) { var p = P[i]; p.y -= p.vy * dt; p.ph += p.sway * dt; var x = p.x + Math.sin(p.ph) * p.amp;
      ...
      ctx.globalAlpha = a < 0 ? 0 : a; ctx.drawImage(p.cool ? aether : ember, x - sz / 2, p.y - sz / 2, sz, sz); }
    ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over"; raf = requestAnimationFrame(frame);
  }
  function start() { if (!raf) { last = 0; raf = requestAnimationFrame(frame); } }
  function stop() { if (raf) { cancelAnimationFrame(raf); raf = null; } }
  if (reduce) { resize(); ctx.globalCompositeOperation = "lighter"; for (var i = 0; i < 24; i++) { var sz = rnd(5, 12); ctx.globalAlpha = rnd(.05, .14); ctx.drawImage(ember, rnd(0, W), rnd(0, H), sz, sz); } ctx.globalAlpha = 1; }
  else { init(); start(); }
