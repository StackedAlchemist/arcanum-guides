/* ============================================================
   THE ARCANUM - Final Fantasy IV guide - shared site JS
   Ambient moonlit motes, reveal-on-scroll, mobile nav, top.
   ============================================================ */

/* --- ambient: drifting moonlit motes (silver-blue + gold) --- */
(function () {
  ...
  var moon = spr("196,216,245"), gold = spr("216,182,90");
  function mk(init) { return { x: rnd(0, W), y: init ? rnd(0, H) : H + 10, r: rnd(1, 2.6), vy: rnd(6, 16), sway: rnd(.3, .9), amp: rnd(8, 22), ph: rnd(0, 6.28), gold: Math.random() < 0.22 }; }
  ...
  function frame(t) {
    ...
    for (var i = 0; i < P.length; i++) { var p = P[i]; p.y -= p.vy * dt; p.ph += p.sway * dt; var x = p.x + Math.sin(p.ph) * p.amp;
      ...
      ctx.globalAlpha = a < 0 ? 0 : a; ctx.drawImage(p.gold ? gold : moon, x - sz / 2, p.y - sz / 2, sz, sz); }
    ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over"; raf = requestAnimationFrame(frame);
  }
  function start() { if (!raf) { last = 0; raf = requestAnimationFrame(frame); } }
  function stop() { if (raf) { cancelAnimationFrame(raf); raf = null; } }
  if (reduce) { resize(); ctx.globalCompositeOperation = "lighter"; for (var i = 0; i < 24; i++) { var sz = rnd(5, 12); ctx.globalAlpha = rnd(.05, .14); ctx.drawImage(moon, rnd(0, W), rnd(0, H), sz, sz); } ctx.globalAlpha = 1; }
  else { init(); start(); }
